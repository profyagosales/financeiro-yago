...
// ─── Background mesh with animated orbs ──────────────────────────
function BackgroundMesh() {
  return (
    <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 0, overflow: 'hidden' }}>
      {[
        { x: '15%', y: '10%', color: 'rgba(196,85,59,0.08)', size: 500, delay: 0 },
        { x: '75%', y: '20%', color: 'rgba(58,133,128,0.07)', size: 420, delay: 2 },
        { x: '50%', y: '70%', color: 'rgba(212,160,23,0.05)', size: 380, delay: 4 },
      ].map((orb, i) => (
        <motion.div
          key={i}
...
export function DashboardPage() {
  ...
  // ── Calculations ──
  const fixasPendentes = contasFixas.filter(
    cf => !pagamentos.find(p => p.contaFixaId === cf.id && p.status === 'pago'),
  )
  const fixasPagas = contasFixas.filter(
    cf => pagamentos.find(p => p.contaFixaId === cf.id && p.status === 'pago'),
  )
  ...
  const proximosVenc = contasFixas
    .filter(cf => {
  ...
  return (
    <>
      ...
      <motion.div
        ...
        initial="hidden"
        animate="show"
        style={{ padding: '24px 28px', width: '100%', maxWidth: 900, position: 'relative', zIndex: 1 }}
      >
        {/* ─── Page header ─── */}
        <motion.div variants={I} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 13, color: '#9B7B6A', marginBottom: 2 }}>
              {saudacao}, Yago
            </p>
            <p style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 14, fontWeight: 700, color: '#2C1A0F' }}>
          ...
          <div style={{
            ...
            <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 12, fontWeight: 600, color: '#7A5C4F', textTransform: 'capitalize' }}>
              {mesNome}
            </p>
          </div>
        </motion.div>
        ...
        >
          ...
          {/* Breakdown */}
          {[
            { icon: IconTrendingDown, label: 'Gastos variáveis', valor: despesas, sub: undefined },
            {
              icon: IconRepeat,
              label: `Contas fixas (${contasFixas.length})`,
              valor: totalFixasMes,
              sub: `${fixasPagas.length} pagas · ${fixasPendentes.length} pendentes`,
            },
            { icon: IconCalendarStats, label: `Parcelamentos (${parcelamentos.length})`, valor: totalParcelamentos, sub: undefined },
          ]
            ...
            .map((item, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 12, color: 'rgba(255,255,255,0.42)', flex: 1 }}>
                  {item.label}
                  {item.sub ? <span style={{ opacity: 0.7 }}> · {item.sub}</span> : ''}
                </span>
                <span style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.65)' }}>
                  {fmt(item.valor)}
                </span>
              </div>
            ))}
        ...
        {/* ─── Contas (horizontal scroll) ─── */}
        {contas.length > 0 && (
          <motion.div variants={I} style={{ marginBottom: 18 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <h2 style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 20, fontWeight: 700, color: '#2C1A0F' }}>
                Contas
              </h2>
              <button
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 12 }}>
                    <div style={{
                      ...
                      <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 9, fontWeight: 800, color: 'white', letterSpacing: '-0.3px' }}>
                        {c.icone}
                      </span>
                    </div>
                    <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, fontWeight: 600, color: '#7A5C4F', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {c.nome}
                    </p>
                  </div>
                  <OdometroSaldo
                  ...
                  />
                  <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 10, color: '#C4B4A8', marginTop: 3, textTransform: 'capitalize' }}>
                    {c.tipo}
                  </p>
                </motion.div>
              ))}
        ...
        {/* ─── Bento grid: gastos + alertas ─── */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16, marginBottom: 18 }}>

          {/* Gastos por categoria */}
          {pieData.length > 0 && (
            <motion.div variants={I} style={{ ...CARD, padding: '18px 20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <h2 style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 18, fontWeight: 700, color: '#2C1A0F' }}>
                  Gastos por categoria
                </h2>
                <button
          ...
          {/* Alertas column (vencimentos + cartões) */}
          {hasAlerts && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>

              {/* Vencimentos próximos */}
              {proximosVenc.length > 0 && (
                <motion.div variants={I} style={{ ...CARD, padding: '16px 18px', borderColor: '#F5DFA0' }}>
              ...
              {cartoesAlerta.length > 0 && (
                <motion.div variants={I} style={{ ...CARD, padding: '16px 18px', borderColor: '#F5C8B8' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
