...
import { IconEdit, IconTrash, IconPaperclip, IconX, IconSearch, IconClock, IconCheck, IconFilter, IconArrowUpRight, IconArrowDownRight, IconArrowsExchange } from '@tabler/icons-react'
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            <h3 style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 22, fontWeight: 700, color: '#2C1A0F' }}>Editar lançamento</h3>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 10px', borderRadius: 20, background: tx.tipo === 'receita' ? '#EBF5F0' : '#FAF0EE', marginTop: 6 }}>
              ...
              <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, fontWeight: 700, color: tx.tipo === 'receita' ? '#3A8580' : '#C4553B' }}>
        ...
        <input value={desc} onChange={e => setDesc(e.target.value)} placeholder="Descrição"
          style={{ width: '100%', background: '#FAF6F0', border: '1.5px solid #E8E0D5', borderRadius: 13, padding: '12px 15px', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 14, color: '#2C1A0F', outline: 'none', marginBottom: 10, boxSizing: 'border-box' }} />
        <input value={data} onChange={e => setData(e.target.value)} type="date"
          style={{ width: '100%', background: '#FAF6F0', border: '1.5px solid #E8E0D5', borderRadius: 13, padding: '12px 15px', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 14, color: '#2C1A0F', outline: 'none', marginBottom: 14, boxSizing: 'border-box' }} />

        {contas.length > 0 && (
          <div style={{ marginBottom: 14 }}>
            <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, fontWeight: 700, color: '#9B7B6A', marginBottom: 8, letterSpacing: '.06em' }}>CONTA</p>
            <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
              {contas.map(c => (
                <button key={c.id} onClick={() => setContaId(c.id!)}
                  style={{ padding: '7px 13px', borderRadius: 20, border: contaId === c.id ? `2px solid ${c.cor}` : '1.5px solid #E8E0D5', cursor: 'pointer', background: contaId === c.id ? `${c.cor}18` : 'white', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 12, fontWeight: 600, color: contaId === c.id ? c.cor : '#7A5C4F', display: 'flex', alignItems: 'center', gap: 5, transition: 'all .15s' }}>
        ...
        <div style={{ marginBottom: 22 }}>
          <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, fontWeight: 700, color: '#9B7B6A', marginBottom: 10, letterSpacing: '.06em' }}>CATEGORIA</p>
        ...
        <motion.button onClick={handleSave} whileTap={{ scale: 0.97 }}
          style={{ width: '100%', padding: '15px 0', borderRadius: 15, border: 'none', cursor: 'pointer', background: '#C4553B', color: 'white', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 15, fontWeight: 700, boxShadow: '0 4px 16px rgba(196,85,59,0.3)' }}>
          Salvar alterações
        </motion.button>
      </motion.div>
    </motion.div>
    ...
    <>
      <motion.div
        ...
        style={{
          ...
          background: showActions ? `rgba(${hexToRgb(catCor)}, 0.07)` : '#FFFDF9',
          border: showActions ? `1.5px solid rgba(${hexToRgb(catCor)}, 0.35)` : '1px solid #EDE6DC',
          transition: 'all .2s',
        }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 13, padding: '13px 15px' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 6, marginTop: 3, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              {conta && (
                <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 10, background: `rgba(${hexToRgb(conta.cor)},0.12)`, color: conta.cor, padding: '2px 7px', borderRadius: 20, fontWeight: 600 }}>{conta.nome}</span>
              )}
              {tx.status === 'pendente' && (
                <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 10, background: '#FDF4E3', color: '#D4A017', padding: '2px 7px', borderRadius: 20, fontWeight: 700, border: '1px solid #F0D8A8', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
              ...
              <div style={{ display: 'flex', borderTop: `1px solid rgba(${hexToRgb(catCor)},0.15)` }}>
                {[
                  ...
                  <button key={bi} onClick={e => { e.stopPropagation(); btn.action() }}
                    style={{ flex: 1, padding: '11px 0', border: 'none', background: btn.bg, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 12, fontWeight: 600, color: btn.color, borderRight: bi < 2 ? `1px solid rgba(${hexToRgb(catCor)},0.12)` : 'none' }}>
                    {btn.icon}{btn.label}
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <motion.button whileTap={{ scale: 0.95 }} onClick={() => fileRef.current?.click()}
                    style={{ background: '#C4553B', color: 'white', border: 'none', borderRadius: 11, padding: '8px 14px', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>+ Adicionar</motion.button>
    ...
    <div style={{ padding: '24px 28px', width: '100%' }}>
        ...
        <div>
          ...
          {contaFiltrada && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
              ...
              <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 12, fontWeight: 600, color: contaFiltrada.cor }}>{contaFiltrada.nome}</span>
              <button onClick={() => navigate('/transacoes')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, color: '#9B7B6A', padding: 0 }}>× limpar</button>
            </div>
          )}
        </div>
        <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 12, color: '#9B7B6A', background: '#F5F0E8', padding: '5px 12px', borderRadius: 20, fontWeight: 600 }}>{filtradas.length} itens</span>
      </div>

      {/* Search bar */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#FFFDF9', border: '1.5px solid #E8E0D5', borderRadius: 16, padding: '11px 15px', marginBottom: 14 }}>
        <IconSearch size={16} color="#9B7B6A" stroke={1.8} />
        <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar transações..."
          style={{ flex: 1, border: 'none', background: 'transparent', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 14, color: '#2C1A0F', outline: 'none' }} />
        {busca && <button onClick={() => setBusca('')} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex' }}><IconX size={14} color="#C4B4A8" /></button>}
      </div>

      {/* Month filter */}
      <div style={{ display: 'flex', gap: 6, marginBottom: 12, overflowX: 'auto', paddingBottom: 4 }}>
        <motion.button whileTap={{ scale: 0.95 }} onClick={() => setFiltroMes(null)}
          style={{ padding: '7px 14px', borderRadius: 20, border: 'none', cursor: 'pointer', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, fontWeight: 700, background: filtroMes === null ? '#2C1A0F' : '#F5F0E8', color: filtroMes === null ? 'white' : '#7A5C4F', whiteSpace: 'nowrap', flexShrink: 0 }}>
          Todos
        </motion.button>
        {Array.from({ length: 12 }, (_, i) => {
          ...
          return (
            <motion.button key={`${a}-${m}`} whileTap={{ scale: 0.95 }} onClick={() => { setFiltroMes(m); setFiltroAno(a) }}
              style={{ padding: '7px 14px', borderRadius: 20, border: active ? 'none' : '1.5px solid #E8E0D5', cursor: 'pointer', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, fontWeight: active ? 700 : 500, background: active ? '#C4553B' : 'transparent', color: active ? 'white' : '#7A5C4F', whiteSpace: 'nowrap', flexShrink: 0, textTransform: 'capitalize', transition: 'all .15s' }}>
              {label}
            </motion.button>
          )
        })}
      ...
      <div style={{ display: 'flex', gap: 7, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', background: '#F5F0E8', borderRadius: 12, padding: 3, gap: 3 }}>
          {([['todos', 'Todos', null], ['receita', '+ Receitas', '#3A8580'], ['despesa', '− Despesas', '#C4553B']] as const).map(([val, label, cor]) => (
            <button key={val} onClick={() => setFiltroTipo(val as any)}
              style={{ padding: '7px 14px', borderRadius: 10, border: 'none', cursor: 'pointer', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 12, fontWeight: 700, transition: 'all .15s',
        ...
        <div style={{ display: 'flex', background: '#F5F0E8', borderRadius: 12, padding: 3, gap: 3 }}>
          {([['todos', 'Todos', null], ['confirmado', null, '#3A8580'], ['pendente', null, '#D4A017']] as const).map(([val, , cor]) => (
            <button key={val} onClick={() => setFiltroStatus(val as any)}
              style={{ padding: '7px 12px', borderRadius: 10, border: 'none', cursor: 'pointer', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, fontWeight: 700, transition: 'all .15s',
        ...
        {hasFilter && (
          <button onClick={() => { setFiltroTipo('todos'); setFiltroStatus('todos'); setFiltroCategoria(null); setFiltroMes(null) }}
            style={{ padding: '7px 12px', borderRadius: 10, border: '1.5px solid #E8E0D5', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, fontWeight: 600, color: '#9B7B6A' }}>
      ...
      {categoriasTodas.length > 0 && (
        <div style={{ display: 'flex', gap: 7, marginBottom: 20, overflowX: 'auto', paddingBottom: 4 }}>
          <motion.button whileTap={{ scale: 0.95 }} onClick={() => setFiltroCategoria(null)}
            style={{ padding: '7px 14px', borderRadius: 20, border: filtroCategoria === null ? 'none' : '1.5px solid #E8E0D5', cursor: 'pointer', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, fontWeight: 700, background: filtroCategoria === null ? '#F5F0E8' : 'transparent', color: filtroCategoria === null ? '#2C1A0F' : '#9B7B6A', whiteSpace: 'nowrap', flexShrink: 0 }}>
            Todas
          </motion.button>
          {categoriasTodas.map(c => (
            <motion.button key={c.id} whileTap={{ scale: 0.95 }} onClick={() => setFiltroCategoria(filtroCategoria === c.id! ? null : c.id!)}
              style={{ padding: '5px 12px 5px 6px', borderRadius: 20, border: filtroCategoria === c.id ? 'none' : '1.5px solid #E8E0D5', cursor: 'pointer', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, fontWeight: 600, background: filtroCategoria === c.id ? c.cor : 'transparent', color: filtroCategoria === c.id ? 'white' : '#9B7B6A', whiteSpace: 'nowrap', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 5, transition: 'all .15s', boxShadow: filtroCategoria === c.id ? `0 2px 8px ${c.cor}40` : 'none' }}>
      ...
      {filtradas.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '56px 0' }}>
          ...
          <p style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 18, fontWeight: 700, color: '#2C1A0F', marginTop: 14 }}>
            ...
            return (
              <div key={data} style={{ marginBottom: 22 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                  <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 12, fontWeight: 700, color: '#9B7B6A', textTransform: 'capitalize', flex: 1 }}>{labelData(data)}</p>
                  <div style={{ height: 1, flex: 2, background: '#EDE6DC' }} />
                  <p style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 13, fontWeight: 700, color: saldoDia >= 0 ? '#3A8580' : '#C4553B', flexShrink: 0 }}>
