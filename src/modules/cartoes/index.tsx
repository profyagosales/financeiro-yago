...
function cardGrad(cor: string) {
  return `linear-gradient(140deg, ${lightenHex(cor, 22)} 0%, ${cor} 50%, ${darkenHex(cor, 30)} 100%)`
}
...
function LancRow({ lanc }: { lanc: any }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: '#FFFDF9', border: '1px solid #EDE6DC', borderRadius: 15 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 5, marginTop: 3, alignItems: 'center' }}>
          {cat && <span style={{ fontSize: 10, background: `${cat.cor}20`, color: cat.cor, padding: '2px 8px', borderRadius: 20, fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 600 }}>{cat.nome}</span>}
...
function FaturaSheet({ cartao, onClose }: { cartao: any; onClose: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      style={{ position: 'fixed', inset: 0, background: 'rgba(20,8,0,0.6)', zIndex: 300, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={onClose}>
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
        transition={{ type: 'spring', stiffness: 300, damping: 30 }}
        onClick={e => e.stopPropagation()}
        style={{ width: '100%', maxWidth: 560, background: '#FFFDF9', borderRadius: '28px 28px 0 0', maxHeight: '90dvh', overflowY: 'auto', overflow: 'hidden' }}>

        {/* Handle */}
        <div style={{ width: 40, height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.4)', margin: '12px auto 0', position: 'relative', zIndex: 1 }} />

        {/* Card header */}
        <div style={{ background: cardGrad(cartao.cor), padding: '14px 22px 22px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', bottom: -30, left: 10, width: 120, height: 120, borderRadius: '50%', background: 'rgba(255,255,255,0.04)', pointerEvents: 'none' }} />

          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, position: 'relative' }}>
            <div>
              <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 9, fontWeight: 700, color: subTxt, letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 4 }}>{cartao.bandeira}</p>
              <h3 style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 22, fontWeight: 700, color: txt, lineHeight: 1 }}>{cartao.nome}</h3>
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <motion.button onClick={() => setAddingLanc(true)} whileTap={{ scale: 0.95 }}
                style={{ background: 'rgba(255,255,255,0.22)', border: '1px solid rgba(255,255,255,0.3)', color: txt, borderRadius: 11, padding: '9px 16px', fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                + Lançar
              </motion.button>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 12, position: 'relative' }}>
            <div>
              <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 9, color: subTxt, letterSpacing: '.07em', textTransform: 'uppercase', marginBottom: 3 }}>FATURA ATUAL</p>
              <p style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 28, fontWeight: 700, color: txt, letterSpacing: '-1px', lineHeight: 1 }}>{fmt(total)}</p>
            </div>
            <div style={{ textAlign: 'right' }}>
              <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 9, color: subTxt, letterSpacing: '.05em', marginBottom: 3 }}>DISPONÍVEL</p>
              <p style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 16, fontWeight: 700, color: disponivel < 0 ? '#FFB347' : txt }}>{fmt(Math.max(0, disponivel))}</p>
            </div>
          </div>

          <div style={{ background: 'rgba(255,255,255,0.15)', borderRadius: 5, height: 5, overflow: 'hidden', position: 'relative' }}>
            <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ type: 'spring', stiffness: 200, damping: 25 }}
              style={{ height: '100%', background: pct > 80 ? '#FFB347' : (light ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.85)'), borderRadius: 5 }} />
          </div>
        </div>

        {/* Month navigator */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 22px 12px', borderBottom: '1px solid #EDE6DC' }}>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <p style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 24, fontWeight: 700, color: '#2C1A0F', letterSpacing: '-0.5px' }}>{fmt(total)}</p>
          </div>
        ...
        {/* Lancamentos */}
        <div style={{ padding: '16px 22px 48px' }}>
...
function AddLancForm({ cartaoId, mes, ano, onClose }: { cartaoId: number; mes: number; ano: number; onClose: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
      style={{ position: 'fixed', inset: 0, background: 'rgba(20,8,0,0.65)', zIndex: 400, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={onClose}>
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} transition={{ type: 'spring', stiffness: 320, damping: 32 }}
        onClick={e => e.stopPropagation()}
        style={{ width: '100%', maxWidth: 560, background: '#FFFDF9', borderRadius: '28px 28px 0 0', padding: '8px 22px 48px', maxHeight: '90dvh', overflowY: 'auto' }}>
...
function CartaoCard({ cartao, mes, ano, onClick, onEdit, onDelete }: { cartao: any; mes: number; ano: number; onClick: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <motion.div
      whileHover={{ y: -5, boxShadow: `0 16px 44px ${cartao.cor}50` }}
      transition={{ type: 'spring', stiffness: 300, damping: 26 }}
      style={{
        background: cardGrad(cartao.cor),
        borderRadius: 24,
        padding: '20px 22px 18px',
        cursor: 'default',
        position: 'relative',
        overflow: 'hidden',
        boxShadow: `0 8px 32px ${cartao.cor}38`,
      }}>
      {/* Decorative */}
      <div style={{ position: 'absolute', top: -50, right: -50, width: 180, height: 180, borderRadius: '50%', background: 'rgba(255,255,255,0.07)', pointerEvents: 'none' }} />
      ...
      {/* Top row: bandeira + actions */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 10, fontWeight: 700, color: subTxt, letterSpacing: '.07em', textTransform: 'uppercase' }}>{cartao.bandeira}</p>
        <div style={{ display: 'flex', gap: 5 }}>
          <button onClick={e => { e.stopPropagation(); onEdit() }}
            style={{ background: light ? 'rgba(0,0,0,0.08)' : 'rgba(255,255,255,0.18)', border: 'none', borderRadius: 9, width: 31, height: 31, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <IconEdit size={13} color={light ? 'rgba(30,15,0,0.65)' : 'rgba(255,255,255,0.8)'} stroke={1.8} />
          </button>
          <button onClick={e => { e.stopPropagation(); onDelete() }}
            style={{ background: light ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.12)', border: 'none', borderRadius: 9, width: 31, height: 31, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <IconTrash size={13} color={light ? 'rgba(30,15,0,0.4)' : 'rgba(255,255,255,0.5)'} stroke={1.8} />
          </button>
        </div>
      </div>
      ...
      <div onClick={onClick} style={{ cursor: 'pointer' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 18 }}>
          <p style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 22, fontWeight: 700, color: txt, letterSpacing: '-0.5px', lineHeight: 1.1 }}>{cartao.nome}</p>
          <div style={{ textAlign: 'right' }}>
            <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 9, fontWeight: 700, color: subTxt, letterSpacing: '.07em', marginBottom: 2 }}>FATURA</p>
            <p style={{ fontFamily: "'Fraunces',Georgia,serif", fontSize: 22, fontWeight: 700, color: txt, letterSpacing: '-0.5px' }}>{fmt(total)}</p>
          </div>
        </div>

        {/* Progress bar */}
        <div style={{ background: light ? 'rgba(0,0,0,0.12)' : 'rgba(255,255,255,0.18)', borderRadius: 5, height: 5, overflow: 'hidden', marginBottom: 10 }}>
          <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ type: 'spring', stiffness: 200, damping: 25 }}
            style={{ height: '100%', background: pct > 80 ? '#FFB347' : (light ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.85)'), borderRadius: 5 }} />
        </div>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 10, background: light ? 'rgba(0,0,0,0.08)' : 'rgba(255,255,255,0.18)', color: txt, padding: '4px 10px', borderRadius: 20, fontWeight: 600 }}>Fecha dia {cartao.diaFechamento}</span>
          <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 10, background: light ? 'rgba(0,0,0,0.08)' : 'rgba(255,255,255,0.18)', color: txt, padding: '4px 10px', borderRadius: 20, fontWeight: 600 }}>Vence dia {cartao.diaVencimento}</span>
        </div>
      </div>
    </motion.div>
  )
      ...
      <AnimatePresence>
        {confirmDelete !== null && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            style={{ position: 'fixed', inset: 0, background: 'rgba(20,8,0,0.6)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        ...
        {adding && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            style={{ position: 'fixed', inset: 0, background: 'rgba(20,8,0,0.6)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              ...
              {/* Card preview */}
              <div style={{ background: cardGrad(form.cor), borderRadius: 18, padding: '16px 18px 14px', marginBottom: 22, position: 'relative', overflow: 'hidden', boxShadow: `0 6px 24px ${form.cor}40` }}>
                <div style={{ position: 'absolute', top: -25, right: -25, width: 100, height: 100, borderRadius: '50%', background: 'rgba(255,255,255,0.07)', pointerEvents: 'none' }} />
