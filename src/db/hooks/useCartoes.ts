...
export function useCartoes() {
  return useLiveQuery(() => db.cartoes.where('ativo').equals(1).toArray(), []) ?? []
}
...
export function useTotalFatura(cartaoId: number, mes: number, ano: number) {
  const lancamentos = useLancamentosCartao(cartaoId, mes, ano)
...
export async function addLancamentoCartao(data: {
  ...
  const firstId = await db.lancamentosCartao.add({
    ...base, valor: valorParcela, parcelaAtual: 1, totalParcelas,
    mes: base.mes, ano: base.ano,
  })
  ...
  for (let p = 2; p <= totalParcelas; p++) {
    ...
    await db.lancamentosCartao.add({
      ...base, valor: valorParcela, parcelaAtual: p, totalParcelas,
      mes: m, ano: a, parcelaPaiId: firstId as number,
    })
  }
  return firstId
}
