...
export interface Conta {
  id?: number
  nome: string
  tipo: 'corrente' | 'poupanca' | 'digital' | 'dinheiro' | 'investimento'
  saldoInicial: number
  saldoAtual: number
  cor: string
  icone: string
...
export interface Transacao {
  ...
  transferId?: string
  recorrencia?: 'unica' | 'mensal' | 'semanal' | 'anual'
  syncId?: string
  updatedAt: number
}
...
class FinanceiroYagoDB extends Dexie {
  ...
  metas!: Table<Meta>
  patrimonio!: Table<PatrimonioItem>
  ...
  constructor() {
    ...
    this.version(1).stores({
      ...
      lancamentosCartao: '++id, cartaoId, mes, ano, parcelaPaiId, syncId',
...
export async function seedCategories() {
  ...
  await db.categorias.bulkAdd([
    ...
    { nome: 'Investimentos', tipo: 'receita', icone: '📊', cor: '#D4A017', ordem: 13 },
